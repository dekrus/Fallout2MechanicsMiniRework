...
При первом запуске после установки мода нужно загрузить любое сохранение, чтобы новые описание трейтов вступили в силу.

Дополняет стандартные особенности новыми свойствами, не меняя их базовую механику. 
Все новые свойства особенностей заскриптованы и будут работать, даже если сами особенности были изменены игроком или другими модами.
Все новые свойства для конкретной особенности могут быть включены/отключены или принудительно активированы, даже если самой особенности у игрока нет, при включенной опции в .ini файле настроек.

Быстрый метаболизм:
++ Вся еда и химия выводят на 33% больше яда и радиации.
+- Вся еда и химия лечат/снимают на 33% больше ХП.
+- Вся еда и химия срабатывают и заканчивают своё действие в 2 раза быстрее.
+- Если включен "RegenMod", эффект регенерации здоровья увеличивается на 33%. Однако, если игрок отравлен, её эффективность снижается на 50%. Степень уменьшения зависит от сопротивления яду. Например, при сопротивлении яду в 90%, сила регенерации при отравлении ослабляется лишь на 5%.
#Здоровяк:
++ Максимальный урон от безоружных атак, оружия ближнего боя и метательного оружия (кроме гранат) удваивается.
++ Значение характеристики "Физич. Урон" теперь учитывается при расчёте максимального урона метательного оружия.
#Миниатюрный:
++ Максимальный шанс попадания по игроку уменьшен на 10%, учитывает любые другие снижения максимального шанса попадания по игроку из других модов/перков.
++ Навык Незаметности на 10% эффективнее.
#Однорукий:
++ Безоружные атаки одной рукой теперь также получают бонус в +20% к шансу попадания.
++ Возможность атаковать одноручным оружием, даже если обе руки травмированы (бонус к шансу попадания не применяется при травме обеих рук).
++ Бонус +5% к силе критического удара.
++ Одноручное оружие требует на 3 силы меньше для правильного использования.
#Точность:
+  Атаки Игрока наносящие урон площади теперь могут наносить критический урон, пробивать броню и нокаутировать дополнительные цели.
+  Влияет на все виды атаки по площади: атаки очередью, гранаты, огнемёты и альтернативные режимы стрельбы.
#Камикадзе:
(совместимо с "AutoMoveToAttack" sfall 5.0+)
+  Игрок может "атаковать на ходу", снижая стоимость атаки на 1 ОД за каждые 2 шага, до минимума в 1 ОД для неприцельной и 2 ОД для прицельной атаки.
+  "Реакция" увеличена на 10 единиц, вместо 5.
+  Альтернативный режим стрельбы из пистолета не прерывется при движении.
-  Каждый пройденый шаг уменьшает шанс попадания на 3%. После атаки и в конце хода штраф к меткости обнуляется.
#Тяжелая рука:
+  Половина значения характеристики "Физич. Урон"(с округлением вверх) прибавляется к минимальному урону при расчёте урона от безоружных атак, оружия ближнего боя, и метательного оружия (не гранаты).
+  Значение характеристики "Физич. Урон" теперь учитывается при расчёте максимального урона метательного оружия.
+  Каждая не критическая атака оружием ближнего боя и метательным оружием может сбить цель с ног. Шанс зависит от урона, силы игрока, веса оружия, а также от типа противника и его выносливости.
+  Тяжесть критических промахов игрока снижается на 30 ед.
#Кровавая баня:
+  Первое убийство в каждом ход восстанавливает игроку 2 ОД, каждое следующее убийство в этом ходу восстанавивет 1 ОД.
+- Тяжесть критических промахов для всех, включая игрока увеличена на 30 единиц. 
#Порченый:
+- Снижает максимальный шанс попадания на 5% для игрока и НПЦ.
+- Тайм лимит перманентно отключен. Критические промахи начинают работать с первой минуты игры.
#Чувст. к химии:
+- Положительное и отрицательное изменение характеристик от химии и еды усиленно на 50% с округленим до ближайшего целого (-1 -> -2|+1 -> +2).
#Сопрот. к химии:
+ Еда и химия больше не вызывают зависимости.
+ Характеристики возвращаются в норму сразу после окончания действия первого эффекта химии и еды.
+ Можно принять вполовину больше доз. Если было ограничение в 1 дозу, то теперь можно будет принять 2.
+- Положительное и отрицательное изменение характеристик от химии и еды ослаблено на 50% с округлением вверх (-1 -> 0|+1 -> +1).
#Тренированный:
+ Увеличивает максимальный шанс попадания на 4%.
+ Критический промах с вероятностью 50% станет обычнм промахом.
+ Снижает требования к уровню для выбора перков на 4. Перки, требующие 9 уровень, можно взять на 5 уровне, а перки, требующие 12 уровень, можно взять на 8 уровне.

### Альтернативный режим стрельбы
(Требуется выставить "AllowUnsafeScripting = 1 или 2" в ddraw.ini)!!!
Этот модуль добавляет Лазерному, Плазменному и Гаусс оружию, а также пистолетам, ПП и дробовикам альтернативный режим стрельбы.
...
Импульсное и ЭМИ оружие дальнего боя:
...
- Все НПЦ и Компаньоны, когда отравлены, могут применять антидот (или стимуляторы, если  нет антидота). Работает как в бою, так и вне боя и даже при перемещении по глобальной карте и во время отдыха.
...
- Стимуляторы мгновенно восстанавливают  1-3 ед. здоровья и на 15 секунд (3 хода в бою) увеличивают скорость восстановления на 20 ед., в конце своего действия снижая скорость восстановления на 25 ед. на 1 минуту.
...
Основные моменты:
- В случае неудачи кражи вместо начала боя будет воспроизведена одна из "сценок" и наложен временный штраф на продажу, покупку, а также на проверку навыков красноречия и бартера в диалогах.
...
- Жертвы неудачной кражи или неудачной попытки заглянуть в карман будут пару дней помнить, что игрок уже пытался у них украсть, и в следующий раз их реакция будет жестче.
- При повышении количества неудачных краж выше лимита все НПЦ в городе будут реагировать на попытки украсть у них жестче.
- Убийство всех свидетелей кражи до распространения слухов или до перехода на новую карту предотвращает увеличение счётчика неудачных краж в городе.

- Расчет вероятности успеха аналогичен исходной формуле с добавлением зависимости от восприятия цели и навыка кражи игрока.
- Если навык кражи развит более 140%, то за каждое 1 очко сверху максимальный шанс успешной кражи увеличивается на 0.1% до максимума в 99%.
...
1. **Ножи**:
    - **Режущая атака**:
      - Имеет увеличенный шанс попадания и зависит от ловкости цели и навыков ближнего боя атакующего.
      -  По слабо бронированным целям режущая атака с большей вероятностью нанесет максимальный урон, а не минимальный.
    - **Колющая атака**:
      - Пробивает часть сопротивления урону (DR) и порога урона (DT) цели в зависимости от показателей максимального урона ножа и бонусного урона в ближнем бою атакующего.
      - Увеличивает базовый шанс критического удара в 1,5 раза.

2. **Кувалды, дубинки, погонялки для скота**:
    - **Взмах**:
      - Больше не отбрасывают существ при попадании и имеют шанс сбить с ног в зависимости от нанесенного урона.
    - **Толчок**:
      - Имеет увеличенную дальность атаки.
      - Наносит до 40% больше урона целям, которые прижаты к другому объекту (например, стенам, существам или пейзажу) и не могут быть отброшены дальше, или если они ударяются об объект во время «скольжения».
